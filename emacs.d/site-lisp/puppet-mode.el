      ...
      (cond
       ...
       (array-start
        (save-excursion
          ...
          (forward-char 1)
	  ;; Are we looking at whitespace up until the end of the line?
	  ;;
	  ;; $foo = [-!-.\n
	  ;;
          (if (looking-at "\\s-+\n")
              (setq cur-indent (+ (current-column) (puppet-indent-level)))
	    ;; Not at \s$.... Move to first non-whitespace character
            (re-search-forward "\\S-")
	    ;; Now, move back 2
            (forward-char -2)
            (setq cur-indent (+ (current-column) puppet-indent-level)))))
